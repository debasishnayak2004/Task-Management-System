            ...
            <div class="container mt-4">
                ...
                <div class="row">
                    <div class="col-8 offset-2">
                        <?php
                        $id = $_GET["id"];
                        $sql1 = "select * from `task` where id = $id";
                        $result1 = mysqli_query($conn, $sql1);
                        $data = mysqli_fetch_assoc($result1);
                        ?>
                    <form action="task_update_action.php" method="post">
                    <input type="hidden" name="id" value="<?php echo $data["id"] ;?>" class="form-control" id="id"  required>
                            <div class=" mb-2">
                                ...
                                <select class="form-select mt-2" name="user_id" id="user_id" required>
                                    ...
                                    ?>
                                    <option <?php echo $data["user_id"] == $row["id"] ? "selected" : ""?> value="<?php echo $row["id"]  ?>"><?php echo $row["name"]; ?></option>
                                    <?php
                                        }
                                    ?>
                                </select>
                            ...
                            <div class=" mb-2">
                                ...
                                <textarea class="form-control mt-2"  rows="5" placeholder="Enter Task Message...." id="message" name="message" required><?php echo htmlspecialchars($data["message"]) ;?></textarea>
                            </div>
                            <div class="mb-2 ">
                                <label for="studentName" class="form-label">Start Date :</label>
                                <input type="date" name="start_date" value="<?php echo $data["start_date"] ;?>" class="form-control" id="start_date" placeholder="Enter Expiry Date"  required>
                            </div>
                            <div class="mb-2 ">
                                <label for="studentName" class="form-label">End Date :</label>
                                <input type="date" name="end_date" value="<?php echo $data["end_date"] ;?>" class="form-control" id="end_date" placeholder="Enter Expiry Date"  required>
                            </div>
